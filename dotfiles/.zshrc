...
plugins=(
  ...
  #! GIT AND STUFF #############################################
  git
  # git-hubflow # not as interesting as both options bellow, but is more integrated with github
  # git-flow    # holy shit i really wanna get in on this
  git-flow-avh
    #! REMEMBER THAT THE GIT FLOW TOOL HAS TO BE INSTALLED SEPARATELY
    # the same as git-flow, but has "feature delete", publishing features
    # (to work with other people at the same time) and other stuff
  ...
  yarn
  rails
  # react-native  # dont currently use. might look into it
  # rvm           # dont currently use. might look into it
  # scala         # dont currently use. might look into it
)
